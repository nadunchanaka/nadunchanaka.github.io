...
import { CATEGORY_CONFIG, MONTH_NAMES } from '../utils/constants';
import Icon from './Icon';

export default function RecentTransactions({ transactions, selectedMonth, selectedYear }) {
    ...
    <section style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <h2 style={{ fontSize: '20px', fontWeight: 700, margin: 0, color: 'var(--color-on-surface)' }}>Recent Transactions</h2>
      ...
      >
        {recent.length === 0 ? (
          ...
          recent.map((t, idx) => {
            ...
            const amtDisplay = isIncome ? `+ LKR ${amt.toLocaleString('en-US')}` : `- LKR ${amt.toLocaleString('en-US')}`;
